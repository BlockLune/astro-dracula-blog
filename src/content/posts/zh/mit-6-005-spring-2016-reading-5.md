...
- 了解方法规范中的前置条件和后置条件，并能编写正确的规范
- 能够根据规范编写测试
- 知道 Java 中已检查异常和未检查异常的区别
- 了解如何使用异常处理特殊结果

## 简介

规范是团队合作的关键。如果没有规范，就不可能将实现方法的工作委派出去。规范就像一份合同：实现者有责任履行合同，而方法的使用者可以依赖这份合同。事实上，我们会发现，就像真正的法律合同一样，规范对双方都提出了要求：当规范有前置条件时，使用者也有责任。

在本篇阅读材料中，我们将探讨方法规范所扮演的角色。我们将讨论什么是前置条件和后置条件，它们对方法的实现者和使用者分别意味着什么。 我们还将讨论如何使用异常，它是 Java、Python 和许多其他现代语言中的一个重要语言特性，让我们能够使方法的接口更安全，更容易理解。
...
程序中许多最糟糕的错误都源于对两段代码之间接口行为的误解。尽管每个程序员心中都有规范，但并非所有程序员都会将其写下来。因此，团队中的不同程序员心中有不同的规范。当程序出错时，很难确定错误出在哪里。代码中的精确规范可以让您分配责任（给代码片段，而不是人！），并可以避免您苦苦思考修复应该放在何处。

规范对方法的使用者很有用，因为它们可以让您不必再阅读代码。如果您不相信阅读规范比阅读代码更容易，那么请看看一些标准的 Java 规范，并将它们与实现这些规范的源代码进行比较。

下面是一个来自 `BigInteger` 中的一个方法的例子：

[API 文档中的规范](https://docs.oracle.com/javase/8/docs/api/java/math/BigInteger.html#add-java.math.BigInteger-)：
...
`BigInteger.add` 的规范对于用户来说简单明了，而且如果我们对边界情况有疑问，`BigInteger` 类还提供了额外的人类可读文档。 如果只有代码，我们就必须从 `BigInteger` 构造函数、`compareMagnitude`、`subtract` 和 `trustedStripLeadingZeroInts` 开始阅读。

规范对方法的实现者有好处，因为规范让实现者可以在不告诉用户的情况下自由改变实现。规范还可以使代码更快。我们将看到，使用较弱的规范可以排除方法可能被调用的某些状态。这种对输入的限制可能会让实现者跳过不再需要的昂贵检查，而使用更高效的实现。

合同就像使用者和实现者之间的 _防火墙_。它使使用者免受单元工作细节的影响 —— 如果您有程序的规范，您就不需要阅读程序的源代码。它也使实现者免于了解单元的使用细节；实现者不必询问每个用户打算如何使用该单元。这道防火墙实现了 _解耦 (decoupling)_，允许独立更改单元代码和客户端代码，只要更改符合规范 —— 各自遵守自己的义务。
...
等价的概念存在于观察者 —— 即用户 —— 的眼中。为了能够用一种实现替代另一种实现，并知道何时可以接受这种替代，我们需要一种规范，准确告诉用户需要依赖什么。
...
Java 有一个[文档注释](https://en.wikipedia.org/wiki/Javadoc)惯例，其中参数由 `@param` 子句描述，结果由 `@return` 和 `@throws` 子句描述。在可能的情况下，应将前置条件放在 `@param` 中，将后置条件放在 `@return` 和 `@throws` 中。 因此，形如这样的规范：
...
方法的规范可以讨论方法的参数和返回值，但绝不应涉及方法的局部变量或方法类的私有字段。你应该考虑使具体实现对规范的读者不可见。
![这张图片展示了以下事实：规范中可以讨论参数和返回值，但不应该涉及方法的局部变量、实现细节和私有字段](https://ocw.mit.edu/ans7870/6/6.005/s16/classes/06-specifications/figures/firewall-talk.png)

在 Java 中，方法的源代码通常无法提供给规范的读者，因为 Javadoc 工具会从您的代码中提取规范注释并将其呈现为 HTML。

### 测试和规范

在测试这一块，我们讨论那些根据规范选择的 _黑盒测试_，以及根据实际实现选择的 _白盒测试_（具体见 [阅读材料 3：测试](https://ocw.mit.edu/ans7870/6/6.005/s16/classes/03-testing/#blackbox_and_whitebox_testing)）。 但重要的是要注意，即使是**白盒测试也必须遵循规范**。您的实现可能提供比规范要求更强的保证，或者它可能具有规范未定义的特定行为。但您的测试用例不应依赖于该行为。测试用例必须遵守合同，就像每个其他使用者一样。

例如，假设您正在测试下面这个，和之前我们用的那个略有不同的 `find` 的规范：
...
这个规范在某种意义上有一个强前置条件：`val` 必须能被找到；而在某种意义上它有一个相当弱的后置条件，即如果数组中 `val` 出现超过一次，这个规范对应该返回哪个 `val` 的索引没有任何说明。即使您实现的 `find` 总是返回最低索引，您的测试用例也不能假定特定的行为：
...
那么，如果白盒测试不能超出规范，那它意味着什么？意味着您正在尝试在一种不受具体实现限制的情况下，找到新的测试样例用以锻炼实现的各个部分。
...
我们当时谈到了 _单元测试_ 的概念，即我们应该独立地为程序的每个模块编写测试代码。一个好的单元测试专注于一个单一的规范。我们的测试几乎总是依赖于 Java 库方法的规范，但是我们编写的一个方法的单元测试不应该在 _别的_ 方法未能满足其规范时失败。正如我们在示例中看到的，一个 `extractWords` 的测试不应该在 `getWebPage` 不满足其后置条件时失败。

良好的 _集成测试_，即使用多个模块的测试，将确保我们的不同方法具有兼容的规范：调用者和实现者如对方期望的那样传递和返回值。集成测试不能替代系统性地设计过的单元测试。举个例子，如果我们只通过调用 `makeIndex` 来测试 `extractWords`，我们只会在其输入空间的一个可能很小的部分上进行测试：这些输入是 `getWebPage` 的可能输出。这样做，我们留下了一个隐藏 bug 的地方，当我们在程序的其他地方使用 `extractWords` 用于不同目的时，或者当 `getWebPage` 开始返回以新格式编写的网页时，bug 就会跳出来。

### 可变方法的规范

我们之前讨论过可变对象和不可变对象，但我们的 `find` 规范例子并没有给我们机会在后置条件中描述副作用（对可变数据的修改）。
...
这份规范是从 Java [`List`](https://docs.oracle.com/javase/8/docs/api/?java/util/List.html) 接口的规范轻度简化而来的。首先，看看后置条件。它给出了两个约束：第一个告诉我们 `list1` 如何被修改，第二个告诉我们返回值是如何确定的。

其次，看看前置条件。它告诉我们，将列表元素添加到它本身这样的行为是未定义的。您可以很容易地想到为什么方法的实现者希望施加这种约束：想不到这样的行为有什么应用，并且这样可以让实现更为容易。规范允许一个简单的实现，在这个实现中，您从 `list2` 中取出一个元素并将其添加到 `list1`，然后继续到 `list2` 的下一个元素，直到到达末尾。如果 `list1` 和 `list2` 是同一列表，则此算法将不会终止 —— 这属于规范允许的范畴，因为它的前提条件。
...
正如我们所说的，除非另有说明，否则隐式禁止 `null`。我们也将使用下面的约定：**除非另有说明，否则不允许使用可变对象**。`toLowerCase` 的规范显式地说明了一个 _作用（effect，后置条件）_ 是“`lst` 并为被修改”，但在一个缺失描述可变性的后置条件的情况下，我们要求不改变输入。

---

## 异常

既然我们正编写规范，并考虑用户将如何使用我们编写的方法，那么也让我们来讨论一下，如何以一种既避免出现错误又易于理解的方式来处理 _异常（Exceptional Cases）_ 情况。

一个方法的 _签名（Signature）_ —— 包括其名称、参数类型、返回类型 —— 是其规范的核心部分，而签名中还可能包含该方法可能触发的 _异常（Exception）_。
...
规范在程序实现者与程序使用者之间起到关键的防火墙作用。它使独立开发成为可能：使用者可以自由编写使用该程序的代码而无需查看其源代码，而实现者也可以自由编写实现该程序的代码而无需了解其具体使用方式。

让我们回顾一下规范说明如何帮助实现本课程的主要目标：
